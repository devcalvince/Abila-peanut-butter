...
const OrderModal = ({ isOpen, onClose }: OrderModalProps) => {
  ...
  const [orderData, setOrderData] = useState({
    ...
    paymentMethod: "",
    items: [
      { id: 1, name: "400g Smooth Peanut Butter", price: 300, quantity: 1 },
    ]
  });
  ...
  const shippingCost = orderData.location.toLowerCase().includes('eldoret') && total >= 600 ? 0 : 150;
...
Total: KSh ${total + shippingCost}

Payment Method: ${orderData.paymentMethod}
    `.trim();

    // Here you would normally send the order via email/WhatsApp
          ...
          <div className="border-t pt-4">
            <div className="space-y-2">
              ...
              <div className="flex justify-between font-bold text-lg border-t pt-2">
                <span>Total:</span>
                <span>KSh {total + shippingCost}</span>
              </div>
            </div>
          </div>
